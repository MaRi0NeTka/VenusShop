...
class CustomUserLoginForm(AuthenticationForm):
    username = forms.EmailField(label='Email', widget=forms.EmailInput(attrs={'class': 'dotted-input w-full py-3 text-sm font-medium text-gray-900 placeholder-gray-500', 'placeholder': 'EMAIL'}))
    ...
    def clean(self):
        ...
        for field in ['company', 'address1', 'address2', 'city', 'country',
            ...
            if cleaned_data.get(field):
                cleaned_data[field] = strip_tags(cleaned_data[field]) #обновляем данные после изменения
        return cleaned_data
