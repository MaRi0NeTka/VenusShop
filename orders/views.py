    ...
    def get(self, request):
        ...
        if cart.total_items == 0:
            if request.headers.get('HX-Request'):
                return TemplateResponse(request, 'orders/empty_cart.html', {'message': 'Your cart is empty. Please add items to your cart!'})
    ...
    def post(self, request):
        ...
        if form.is_valid():
            order = Order.objects.create(
                ...
                address2=form.cleaned_data['address'],
            ...
            context = {
                ...
                'cart_items': cart.items.select_related('product', 'product_size__size').order_by('-added_at'),
                'total_price': total_price,
                'error_message': f'Please correct the errors below.'
            }
            if request.headers.get('HX-Request'):
                return TemplateResponse(request, 'orders/checkout_content.html', context)
            return render(request, 'orders/checkout.html', context)
