...
class OrderItemInline(admin.TabularInline):
    ...
    def get_total_price(self, obj):
        try:
            ...
            return mark_safe('<span style="color: red;">Something went wrong</span>')
    get_total_price.short_description = 'Total Price'

@admin.register(Order)
class OrderAdmin(admin.ModelAdmin):
    list_display = ('id', 'user', 'email', 'total_price', 'payment_provider', 'status',  'created_at', 'updated_at')
    list_filter = ('status', 'email', 'first_name', 'last_name')
    ...
    fieldsets = (
        ('Order Information', {
            'fields': (
                ...
                'postal_code', 'phone', 'special_instructions', 'total_price',
            )
        }),
        ('Status & Payment', {
